...
export default function Home() {
  ...
  const avgRating = useMemo(() => {
    ...
    return (s / tutors.length).toFixed(1)
  }, [])

  return (
    <div className="pb-2">
      <div className="hero-gradient px-4 pb-8 pt-6">
        <p className="mb-1 text-xs text-gray-400">✨ {t.subtagline}</p>
        <h1 className="mb-2 text-[22px] font-extrabold leading-tight text-gray-800">{t.tagline}</h1>
        <p className="mb-4 text-xs font-medium text-rose-500/90">{t.homeIntroHint}</p>

        <div className="search-shell mb-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="stat-pill">
            <span className="text-[10px] text-gray-400">{t.homeStatSatisfaction}</span>
            <span className="text-sm font-bold text-gray-800">98%</span>
          </div>
          <div className="stat-pill">
            <span className="text-[10px] text-gray-400">{t.homeStatRating}</span>
            <span className="text-sm font-bold text-gray-800">{avgRating}</span>
          </div>
          <div className="stat-pill">
            <span className="text-[10px] text-gray-400">{t.homeStatTeachers}</span>
            <span className="text-sm font-bold text-gray-800">{tutors.length}</span>
          </div>
        </div>
      </div>

      <div className="page-stack gap-5 !px-4 !pb-24 pt-5">
        <section>
          <h2 className="section-title">{t.homeTrustTitle}</h2>
          <ul className="space-y-2 rounded-3xl border border-pink-100 bg-brand-50/80 p-4">
        ...
        <section>
          ...
          <ol className="flex gap-2">
            {t.homeHowSteps.map((label, i) => (
              <li
                key={i}
                className="flex flex-1 flex-col items-center gap-2 rounded-2xl border border-pink-100 bg-white px-2 py-3 text-center shadow-sm"
              >
                <span className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-rose-500 to-pink-500 text-xs font-extrabold text-white">
                  {i + 1}
                </span>
                <span className="text-[11px] font-semibold leading-snug text-gray-700">{label}</span>
              </li>
            ))}
          </ol>
        </section>
        ...
        <section>
          <div className="mb-3 flex flex-wrap items-center gap-2">
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 rounded-2xl border-[1.5px] border-pink-200 bg-white px-3 py-1.5 text-sm font-semibold text-gray-700"
              >
                <span>↕️</span>
              ...
              {showSort && (
                <div
                  className="absolute left-0 top-11 z-30 min-w-[144px] overflow-hidden rounded-2xl border border-pink-200 bg-white"
          ...
          {filtered.length === 0 ? (
            <p className="rounded-2xl border border-dashed border-pink-200 bg-brand-50/50 py-10 text-center text-sm text-gray-500">
              {t.searchEmpty}
            </p>
          ) : (
